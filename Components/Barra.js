...
function ChefScreen() {
  ...
  const comandasRef = sRef(getDatabase(), `${codigo}/comandasBarra`);
  ...
  const handleEditEstado = (comandaId, estado) => {
    update(sRef(database, `${codigo}/comandasBarra/${comandaId}`), { estado: estado });
  };

  const handleDeleteComanda = (comandaId) => {
    const borrarref = sRef(database,`${codigo}/comandasBarra/${comandaId}`);
    remove(borrarref);
  };
    ...
    return comandas.map((comanda) => (
      <View key={comanda.id} style={styles.comandaContainer}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          ...
          <Text style={{ textAlign: 'center', fontSize: 18, fontWeight: 'bold', marginBottom: 5 }}>{comanda.estado}</Text>
        </View>
        <Text></Text>
